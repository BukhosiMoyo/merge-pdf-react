...
function useStats() {
  ...
  useEffect(() => {
    ...
    DEV && console.debug("🔍 Stats: Starting API call to:", `${API}/v1/merge-pdf/stats`);
    (async () => {
      try {
        const r = await fetch(`${API}/v1/merge-pdf/stats`, { credentials: "include" });
        DEV && console.debug("🔍 Stats: Response status:", r.status, r.ok);
        const j = await r.json().catch(() => ({}));
        DEV && console.debug("🔍 Stats: Response data:", j);
        if (!alive || fetchedRef.current) return;
        const total = Number(j?.total_merged ?? j?.total ?? j?.count);
        if (Number.isFinite(total)) setCount(total);
      } catch (error) {
        DEV && console.warn("🔍 Stats: Error:", error);
      }
    })();
    return () => {
      alive = false;
...
export default function StatsAndFAQ() {
  DEV && console.debug("🔍 StatsAndFAQ: Component rendering, API =", API);
  const total = useStats();
  const agg = useReviews();
  DEV && console.debug("🔍 StatsAndFAQ: total =", total, "agg =", agg);
